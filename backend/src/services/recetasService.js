...
const recetasService = {
  async getAllRecetas() {
    ...
    const recetasConDetalle = await Promise.all(
      recetas.map(async (receta) => {
        const ingredientes = await recetasModel.findDetalleByReceta(
          receta.id_receta,
        );
        return {
  ...
  async getById(id_receta) {
    ...
    if (!receta)
      throw new AppError("Receta no encontrada", httpStatus.NOT_FOUND);
  ...
  async createReceta(nombre_producto, ingredientes) {
    ...
    const idsMaterias = ingredientes.map((ing) => ing.id_materia);
    const hayDuplicados = idsMaterias.length !== new Set(idsMaterias).size;
    if (hayDuplicados)
      throw new AppError(
        "No puede haber materias primas repetidas en la receta",
        httpStatus.BAD_REQUEST,
      );
    ...
    if (Math.abs(total - 100) > 0.01)
      throw new AppError(
        `Los porcentajes deben sumar exactamente 100%. Suma actual: ${total.toFixed(2)}%`,
        httpStatus.BAD_REQUEST,
      );
    ...
    for (const ing of ingredientes) {
      if (parseFloat(ing.cantidad_porcentaje) <= 0)
        throw new AppError(
          "Todos los porcentajes deben ser mayores a cero",
          httpStatus.BAD_REQUEST,
        );
    }
  ...
  async updateReceta(id_receta, nombre_producto, ingredientes) {
    ...
    if (receta.estado === "Inhabilitado")
      throw new AppError(
        "No se puede editar una receta inhabilitada",
        httpStatus.BAD_REQUEST,
      );
  ...
  async inhabilitarReceta(id_receta) {
    ...
    if (receta.estado === "Inhabilitado")
      throw new AppError(
        "La receta ya está inhabilitada",
        httpStatus.BAD_REQUEST,
      );
    ...
    if (tieneOrdenes)
      throw new AppError(
        "No se puede inhabilitar: la receta tiene órdenes de producción activas",
        httpStatus.BAD_REQUEST,
      );
