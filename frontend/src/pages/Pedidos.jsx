                  ...
                  <div className="border-t border-gray-200 pt-4">
                    ...
                    <div className="space-y-2">
                      {pedido.items.filter(Boolean).map((item, idx) => {
                        ...
                        const tieneDevol  = cantDev > 0;
                        return (
                          <div key={idx} className={`p-3 rounded-lg border ${tieneDevol ? "bg-yellow-50 border-yellow-200" : "bg-gray-50 border-gray-100"}`}>
                            <div className="flex items-center justify-between mb-1">
                              <div className="flex items-center gap-2">
                                <Package className="w-4 h-4 text-gray-500 flex-shrink-0" />
                                <span className="text-sm font-medium text-gray-700">{item.nombre_materia}</span>
                              </div>
                            ...
                            {tieneDevol && (
                              <div className="mt-2 pt-2 border-t border-yellow-200 space-y-1">
                                <div className="flex items-center justify-between text-xs">
                                  <span className="flex items-center gap-1 text-red-600 font-medium"><AlertTriangle className="w-3 h-3" />Devolución</span>
                                  <span className="text-red-600 font-bold">{cantDev.toFixed(2)} KG</span>
                                </div>
                                <div className="flex items-center justify-between text-xs">
                                  <span className="flex items-center gap-1 text-green-600 font-medium"><CheckCircle className="w-3 h-3" />Ingresado al inventario</span>
                                ...
                                {item.observacion_devolucion && (
                                  <div className="mt-1 p-2 bg-orange-50 rounded border border-orange-200">
                                    <p className="text-xs text-orange-800"><span className="font-medium">Razón devolución:</span> {item.observacion_devolucion}</p>
                                  </div>
                                )}
                              </div>
                            )}
