...
const dividirNombreDosLineas = (nombre) => {
  ...
  if (!linea1) {
    ...
    return {
      ...
      linea2:
        texto.length > MAX_LINEA * 2
          ? corte2 + "..."
          : texto.slice(MAX_LINEA, MAX_LINEA * 2),
...
const StatusBadge = ({ stockActual, stockMinimo }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-full ${cfg.cls}`}
    >
...
const verificarVinculosInhabilitacion = async (materia) => {
  const bloqueantes = [];
  const advertencias = [];
  let lotesActivos = 0;

  try {
    // 1. Recetas activas — BLOQUEA
    const resRecetas = await fetch(
      `${API_URL}/materias-primas/${materia.id_materia}/recetas-activas`,
    );
    if (resRecetas.ok) {
      ...
      if (Array.isArray(recetas) && recetas.length > 0) {
        bloqueantes.push({
          tipo: "receta",
          mensaje: `Está vinculada a ${recetas.length} receta${recetas.length !== 1 ? "s" : ""} activa${recetas.length !== 1 ? "s" : ""}. Debes desvincularla primero.`,
        });
      }
    }

    // 2. Producciones pendientes o en proceso — BLOQUEA
    const resProd = await fetch(
      `${API_URL}/materias-primas/${materia.id_materia}/producciones-activas`,
    );
    if (resProd.ok) {
      const producciones = await resProd.json();
      if (Array.isArray(producciones) && producciones.length > 0) {
        bloqueantes.push({
          tipo: "produccion",
          mensaje: `Tiene ${producciones.length} producción${producciones.length !== 1 ? "es" : ""} pendiente${producciones.length !== 1 ? "s" : ""} o en proceso que la requieren.`,
        });
      }
    }

    // 3. Pedidos de compra pendientes — BLOQUEA
    const resPedidos = await fetch(
      `${API_URL}/materias-primas/${materia.id_materia}/pedidos-pendientes`,
    );
    if (resPedidos.ok) {
      ...
      if (Array.isArray(pedidos) && pedidos.length > 0) {
        bloqueantes.push({
          tipo: "pedido",
          mensaje: `Tiene ${pedidos.length} pedido${pedidos.length !== 1 ? "s" : ""} de compra pendiente${pedidos.length !== 1 ? "s" : ""} sin completar.`,
        });
      }
    }

    // 4. Historial de movimientos — solo ADVERTENCIA
    const resMov = await fetch(
      `${API_URL}/materias-primas/${materia.id_materia}/movimientos`,
    );
    if (resMov.ok) {
      ...
      if (Array.isArray(movimientos) && movimientos.length > 0) {
        advertencias.push({
          tipo: "movimientos",
          mensaje: `Tiene ${movimientos.length} movimiento${movimientos.length !== 1 ? "s" : ""} registrado${movimientos.length !== 1 ? "s" : ""} en el historial. Estos registros se conservarán para trazabilidad.`,
        });
      }
    }

    // 5. Lotes activos — solo ADVERTENCIA (no bloquea)
    const resLotes = await fetch(
      `${API_URL}/materias-primas/${materia.id_materia}/lotes`,
    );
    if (resLotes.ok) {
      const lotes = await resLotes.json();
      if (Array.isArray(lotes)) {
        lotesActivos = lotes.filter(
          (l) => l.estado !== "agotado" && Number(l.stock_restante) > 0,
        ).length;
        if (lotesActivos > 0) {
          advertencias.push({
            tipo: "lotes",
            mensaje: `Tiene ${lotesActivos} lote${lotesActivos !== 1 ? "s" : ""} activo${lotesActivos !== 1 ? "s" : ""} con stock sin agotar. Puedes registrar una salida en el módulo de Movimientos antes de inhabilitar.`,
          });
        }
      }
    }

    // 6. Stock comprometido — solo ADVERTENCIA
    if (Number(materia.stockComprometido) > 0) {
      advertencias.push({
        tipo: "comprometido",
        mensaje: `Tiene ${Number(materia.stockComprometido).toFixed(2)} kg comprometidos en órdenes relacionadas.`,
      });
    }
  } catch {
  ...
  return { bloqueantes, advertencias, lotesActivos };
};
...
export default function Inventario() {
  ...
  const [modalInhab, setModalInhab] = useState(false);
  const [inhabData, setInhabData] = useState(null);
  const [inhabCargando, setInhabCargando] = useState(false);
  ...
  const [habConfirm, setHabConfirm] = useState(false);

  // ─── Bloquear scroll del body cuando hay un modal abierto ────────────────
  useEffect(() => {
    const hayModalAbierto =
      modalAbierto || modalLotes || modalInhab || modalHab;
    if (hayModalAbierto) {
  ...
  const materiaPrimaSchema = z.object({
    nombre: z
      ...
      .max(
        LIMITES.nombre,
        `El nombre no puede superar ${LIMITES.nombre} caracteres`,
      ),
    ...
    codigo: z
      ...
      .max(
        LIMITES.codigo_max,
        `El código no puede superar ${LIMITES.codigo_max} dígitos`,
      ),
    ...
    stock_min: z.coerce
      ...
      .max(
        LIMITES.stock_max,
        `El stock mínimo supera el límite permitido (${LIMITES.stock_max.toLocaleString("es-CO")} kg)`,
      ),
    ...
    stock_inicial: z.coerce
      ...
      .max(
        LIMITES.stock_max,
        `El stock inicial supera el límite permitido (${LIMITES.stock_max.toLocaleString("es-CO")} kg)`,
      ),
  });
  ...
  const materiasFiltradas = useMemo(
    () =>
      materias.filter((m) => {
        ...
        const coincideStock =
          filtroStock === "todos" || estadoStock === filtroStock;
        const coincideEstado = m.estado === filtroEstado;
        const coincideCat =
          catsFiltro.size === 0 || catsFiltro.has(m.categoria);
  ...
  const estadisticas = useMemo(() => {
    ...
    return {
      ...
      suficiente: activas.filter(
        (m) => calcularEstado(m.stockActual, m.stockMinimo) === "Suficiente",
      ).length,
      bajo: activas.filter(
        (m) => calcularEstado(m.stockActual, m.stockMinimo) === "Bajo",
      ).length,
      critico: activas.filter(
        (m) => calcularEstado(m.stockActual, m.stockMinimo) === "Critico",
      ).length,
    };
  }, [materias]);
      ...
      materia
        ? {
            ...
            id_categoria_materia: materia.id_categoria_materia
              ? String(materia.id_categoria_materia)
              : "",
  ...
  const handleSubmit = async (e) => {
    ...
    const duplicado = materias.find((m) => {
      ...
      return (
        ...
        m.abreviacion?.toLowerCase() ===
          formularioLimpio.abreviacion.toLowerCase()
      );
    });
    ...
    if (duplicado) {
      const erroresDup = {};
      if (
        duplicado.nombre.toLowerCase() === formularioLimpio.nombre.toLowerCase()
      )
      ...
      if (
        duplicado.abreviacion?.toLowerCase() ===
        formularioLimpio.abreviacion.toLowerCase()
      )
        erroresDup.abreviacion =
          "Ya existe una materia prima con esta abreviación";
    ...
    try {
      const url = editando
        ? `${API_URL}/materias-primas/${editando.id_materia}`
      ...
      toast.success(
        editando ? "Materia prima actualizada" : "Materia prima creada",
      );
      cerrarModal();
      cargarMaterias();
    } catch {
  ...
  const abrirModalInhabilitacion = async (materia) => {
    ...
    setInhabObsError("");
    setModalInhab(true);
    setInhabCargando(true);
    try {
      const resultado = await verificarVinculosInhabilitacion(materia);
      setInhabData({ materia, ...resultado });
    } catch {
      toast.error("Error al verificar vínculos. Intenta de nuevo.");
      setModalInhab(false);
    } finally {
      setInhabCargando(false);
    }
  };

  const cerrarModalInhab = () => {
    if (inhabConfirmando) return;
    setModalInhab(false);
    setInhabData(null);
    setInhabObs("");
    setInhabObsError("");
  ...
  const confirmarInhabilitacion = async () => {
    const obsLimpia = inhabObs.trim();
    if (!obsLimpia) {
      setInhabObsError("La observación es obligatoria para inhabilitar.");
      return;
    }
    if (obsLimpia.length < 10) {
      setInhabObsError("La observación debe tener al menos 10 caracteres.");
      return;
    }
    setInhabConfirmando(true);
    try {
      const res = await fetch(
        `${API_URL}/materias-primas/${inhabData.materia.id_materia}`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      const data = await res.json();
      if (!res.ok) {
        toast.error(data.message || "Error al inhabilitar");
        return;
      }
      toast.success(`"${inhabData.materia.nombre}" inhabilitada correctamente`);
      cerrarModalInhab();
      cargarMaterias();
    } catch {
      toast.error("Error de conexión");
    } finally {
      setInhabConfirmando(false);
    }
  };

  // ── Flujo habilitación ────────────────────────────────────────────────
  const abrirModalHabilitacion = (materia) => {
    setHabData(materia);
    setModalHab(true);
  };

  const cerrarModalHab = () => {
    if (habConfirm) return;
    setModalHab(false);
    setHabData(null);
  };
  ...
  const confirmarHabilitacion = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/materias-primas/${habData.id_materia}/habilitar`,
        { method: "PATCH" },
      );
      const data = await res.json();
      if (!res.ok) {
      ...
      toast.success(`"${habData.nombre}" habilitada correctamente`);
      cerrarModalHab();
      cargarMaterias();
    } catch {
      toast.error("Error de conexión");
    ...
    <div className="space-y-4 sm:space-y-6">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            >
              <span className="flex items-center justify-center gap-1.5 sm:gap-2">
                ...
                <span>
                  Inhabilitados (
                  {materias.filter((m) => m.estado === "Inhabilitado").length})
                </span>
              </span>
            </button>
          </div>
            ...
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {[
                  ...
                  "Disponible",
                  "Estado",
                  ...(filtroEstado === "Inhabilitado" ? ["Observación"] : []),
                  "Acciones",
                ].map((col) => (
                  <th
                    key={col}
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {cargando ? (
                ...
                materiasVisibles.map((m) => {
                  const { linea1, linea2, truncado } = dividirNombreDosLineas(
                    m.nombre,
                  );
                  return (
                    <tr key={m.id_materia} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                        <p className="font-medium text-gray-900 text-sm">
                          {Number(m.stockActual).toFixed(2)} kg
                        </p>
                      </td>
                      <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                        <p className="font-medium text-gray-900 text-sm">
                          {Number(m.stockMinimo).toFixed(2)} kg
                        </p>
                      </td>
                      <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                        <p className="font-medium text-amber-600 text-sm">
                          {Number(m.stockComprometido).toFixed(2)} kg
                        </p>
                      </td>
                      <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                        <p className="font-medium text-blue-600 text-sm">
                          {Number(m.stockDisponible).toFixed(2)} kg
                        </p>
                      </td>
                      <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                        <StatusBadge
                        ...
                        <td className="px-4 lg:px-6 py-4 text-center">
                          {m.observacion_inhabilitacion && (
                            <div className="relative group inline-flex justify-center">
                              ...
                              <div className="absolute bottom-full mb-2 hidden group-hover:block z-50 w-72 bg-gray-900 text-white text-xs rounded-lg p-3 shadow-xl">
                                <p className="font-semibold mb-1">
                                  Motivo de inhabilitación:
                                </p>
                                <p>{m.observacion_inhabilitacion}</p>
                              </div>
                            </div>
                          )}
                        </td>
                      ...
                      <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center justify-center gap-2">
                          {filtroEstado === "Activo" ? (
                            ...
                            <button
                              onClick={() => abrirModalHabilitacion(m)}
                ...
                m.observacion_inhabilitacion && (
                  <div className="mb-3 p-2.5 bg-gray-50 rounded-lg border border-gray-200">
                    <p className="text-xs font-semibold text-gray-600 mb-1 flex items-center gap-1">
                      <ClipboardList className="w-3.5 h-3.5" /> Motivo de
                      inhabilitación:
                    </p>
                    ...
                    <div className="flex items-center gap-2">
                      {m.observacion_inhabilitacion && (
                        <div className="relative group">
                          <button
                            className="p-1.5 lg:p-2 text-gray-500 hover:bg-gray-100 rounded-lg transition-colors"
                            title="Ver motivo de inhabilitación"
                          >
                            <ClipboardList className="w-4 h-4" />
                          </button>
                          <div className="absolute right-0 bottom-full mb-2 hidden group-hover:block z-10 w-64 bg-gray-900 text-white text-xs rounded-lg p-3 shadow-xl">
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Numérico único — máx. {LIMITES.codigo_max} dígitos
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={formulario.abreviacion}
                    onChange={(e) =>
                      setField("abreviacion", e.target.value.toUpperCase())
                    }
                    placeholder="Ej: DT"
                    className={`w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm ${
                      errores.abreviacion
                ...
                {(!editando || !loteInicialUsado) && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setField("stock_inicial", e.target.value)
                      }
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Máx. {LIMITES.stock_max.toLocaleString("es-CO")} kg. Crea
                      un lote.
                    </p>
                  </div>
                )}
                ...
                <div className="sm:col-span-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setField("id_categoria_materia", e.target.value)
                    }
                    className={`w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm ${
                  ...
                  >
                    ...
                    {categorias.map((c) => (
                      <option
                        key={c.id_categoria_materia}
                        value={c.id_categoria_materia}
                      >
              ...
              {editando && (
                <p className="text-xs text-gray-500 bg-gray-50 rounded p-2 border border-gray-200 mt-4">
                  ℹ️ El stock actual solo se modifica mediante movimientos de
                  entrada/salida o recepciones de pedidos.
                </p>
              )}
        ...
        >
          <div className="bg-white rounded-2xl shadow-xl w-full sm:max-w-3xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200">
              <div>
                <h2 className="font-bold text-gray-900 text-lg sm:text-xl">
                  Lotes de {materiaLotes.nombre}
                </h2>
                <p className="text-xs sm:text-sm text-gray-600 mt-0.5">
                  Trazabilidad por orden de recepción
                </p>
              </div>
              <button
                onClick={cerrarLotes}
              ...
              {cargandoLotes ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    Los lotes se crean al recibir pedidos
                  </p>
                </div>
              ) : (
                      ...
                      >
                        <div className="flex items-start justify-between mb-3 gap-2">
                          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                            <div
                              className={`w-9 h-9 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${agotado ? "bg-gray-200" : "bg-green-100"}`}
                            >
                              <FileText
                                className={`w-4 h-4 sm:w-5 sm:h-5 ${agotado ? "text-gray-500" : "text-green-600"}`}
                              />
                            </div>
                            <div className="min-w-0">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4 pt-3 border-t border-gray-100">
                          ...
                          <div>
                            ...
                            <p
                              className={`font-bold text-sm ${agotado ? "text-red-600" : porcentaje < 30 ? "text-yellow-600" : "text-green-600"}`}
                            >
                        ...
                        {!agotado && (
                          <div className="mt-3">
                            ...
                            <p className="text-xs text-gray-500 mt-1 text-right">
                              {porcentaje.toFixed(1)}% disponible
                            </p>
                          </div>
                        )}
                      </div>
      ...
      {modalInhab && (
        <div
          ...
          style={overlayStyle}
        >
          <div className="bg-white rounded-2xl shadow-xl w-full sm:max-w-lg max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200">
                ...
                <div>
                  ...
                  {inhabData && (
                    <p
                      ...
                      title={inhabData.materia.nombre}
                    >
                      {inhabData.materia.nombre}
                    </p>
                  )}
                </div>
              </div>
              <button
                onClick={cerrarModalInhab}
                disabled={inhabConfirmando}
            ...
            <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-4">
              {inhabCargando && (
                <div className="flex flex-col items-center justify-center py-10 gap-3 text-gray-500">
                  ...
                  <p className="text-sm">Verificando vínculos activos…</p>
                </div>
              )}

              {!inhabCargando && inhabData && (
                <>
                  {/* BLOQUEANTES */}
                  {inhabData.bloqueantes.length > 0 && (
                    <div className="rounded-xl border border-red-200 bg-red-50 overflow-hidden">
                      <div className="flex items-center gap-2 px-4 py-3 bg-red-100 border-b border-red-200">
                        <XCircle className="w-4 h-4 text-red-600 flex-shrink-0" />
                        <p className="text-sm font-semibold text-red-800">
                          No se puede inhabilitar — vínculos activos
                        </p>
                      </div>
                      <ul className="divide-y divide-red-100">
                        {inhabData.bloqueantes.map((b, i) => (
                          <li
                            key={i}
                            className="flex items-start gap-3 px-4 py-3"
                          >
                            <AlertCircle className="w-4 h-4 text-red-500 flex-shrink-0 mt-0.5" />
                            <p className="text-sm text-red-700">{b.mensaje}</p>
                          </li>
                        ))}
                      </ul>
                      <div className="px-4 py-3 bg-red-50 border-t border-red-100">
                        <p className="text-xs text-red-600 font-medium">
                          Resuelve todos los vínculos anteriores antes de
                          inhabilitar esta materia prima.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {/* ADVERTENCIAS */}
                  {inhabData.advertencias.length > 0 && (
                    <div className="rounded-xl border border-amber-200 bg-amber-50 overflow-hidden">
                      <div className="flex items-center gap-2 px-4 py-3 bg-amber-100 border-b border-amber-200">
                        ...
                        <p className="text-sm font-semibold text-amber-800">
                          {inhabData.advertencias.length > 1
                            ? "Advertencias"
                            : "Advertencia"}{" "}
                      ...
                      <ul className="divide-y divide-amber-100">
                        {inhabData.advertencias.map((a, i) => (
                          <li
                            key={i}
                            className="flex items-start gap-3 px-4 py-3"
                          >
                      ...
                      {inhabData.lotesActivos > 0 && (
                        <div className="px-4 py-3 bg-amber-50 border-t border-amber-100">
                          <p className="text-xs text-amber-700 font-medium flex items-start gap-1.5">
                            <Info className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />
                            Para reducir el stock, ve al módulo de{" "}
                            <span className="underline font-semibold mx-1">
                              Movimientos
                            </span>{" "}
                            y registra una salida.
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    inhabData.advertencias.length === 0 && (
                      <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-xl">
                        ...
                        <p className="text-sm text-green-800 font-medium">
                          No se encontraron vínculos activos. Puedes inhabilitar
                          de forma segura.
                        </p>
                      </div>
                    )}

                  {/* Campo observación — solo si no hay bloqueantes */}
                  {inhabData.bloqueantes.length === 0 && (
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <CharCount value={inhabObs} max={LIMITES.observacion} />
                      </div>
                      <textarea
                        rows={3}
                        ...
                        value={inhabObs}
                        onChange={(e) => {
                          setInhabObs(e.target.value);
                          if (inhabObsError) setInhabObsError("");
                        }}
                        placeholder="Describe el motivo de la inhabilitación (mín. 10 caracteres)…"
                        className={`w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 text-sm resize-none ${
                          inhabObsError ? "border-red-500" : "border-gray-300"
                        }`}
                      />
                      {inhabObsError && (
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Esta observación quedará visible en la sección de
                        inhabilitados del inventario.
                      </p>
                    </div>
                  )}
                </>
              ...
              inhabData.bloqueantes.length > 0 ? (
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    disabled={inhabCargando || inhabConfirmando}
        ...
        >
          <div className="bg-white rounded-2xl shadow-xl w-full sm:max-w-sm p-4 sm:p-6">
            ...
            <p className="text-gray-600 mb-6 text-sm sm:text-base">
              ¿Confirmas que deseas habilitar{" "}
              <span className="font-semibold text-gray-900">
                {habData.nombre}
              </span>
              ? Volverá a estar disponible para su uso en el sistema.
            </p>
            <div className="flex gap-3">
              <button
                onClick={cerrarModalHab}
                disabled={habConfirm}
                className="flex-1 px-4 py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors text-sm font-medium disabled:opacity-50"
              >
                Cancelar
              ...
              >
                {habConfirm ? (
                  ...
                  <>
                    <Check className="w-4 h-4" /> Habilitar
                  </>
                )}
              </button>
            </div>
