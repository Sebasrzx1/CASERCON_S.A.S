    ...
    const fetchMaterias = async () => {
      try {
        ...
        setEstadisticas((prev) => ({
          ...
          stockSuficiente: activas.filter((m) => m.estadoStock === "Suficiente")
            .length,
          stockBajo: activas.filter((m) => m.estadoStock === "Bajo").length,
          stockCritico: activas.filter((m) => m.estadoStock === "Critico")
            .length,
        }));
      } catch (e) {
      ...
      <div className="bg-white border rounded-lg shadow-sm">
        ...
        <div className="p-6 space-y-3">
          {materias
            ...
            .filter(
              (m) => m.estadoStock === "Bajo" || m.estadoStock === "Critico",
            )
            .map((materia) => (
              <div
