...
function ModalConfirmacion({ abierto, titulo, mensaje, onConfirmar, onCancelar, tipo = "danger" }) {
  ...
  const colores = {
    danger:  { icono: "bg-red-100 text-red-600",    boton: "bg-red-600 hover:bg-red-700 text-white" },
    success: { icono: "bg-green-100 text-green-600", boton: "bg-green-600 hover:bg-green-700 text-white" },
  };
  ...
  return (
    <div
      ...
      onClick={(e) => { if (e.target === e.currentTarget && onCancelar) onCancelar(); }}
    >
      <LockScroll />
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-5 sm:p-6">
        <div className="text-center">
          <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center mx-auto mb-4 ${estilo.icono}`}>
            <AlertTriangle className="w-6 h-6 sm:w-7 sm:h-7" />
          </div>
          <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-2">{titulo}</h3>
...
function FormularioOperario({
  formulario, setFormulario, usuarioEditando,
  mostrarContraseña, setMostrarContraseña,
  onSubmit, onCancelar, textoBoton,
}) {
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            onChange={(e) => setFormulario((prev) => ({ ...prev, contraseña: e.target.value }))}
            minLength={5}
            maxLength={8}
          ...
          >
            {mostrarContraseña ? <EyeOff className="w-4 h-4 sm:w-5 sm:h-5" /> : <Eye className="w-4 h-4 sm:w-5 sm:h-5" />}
          </button>
        </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-3 sm:gap-4">
          {["recepcion", "produccion"].map((sector) => (
            <label key={sector} className="inline-flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                value={sector}
              ...
              />
              <span className="text-sm capitalize">{sector === "recepcion" ? "Recepción" : "Producción"}</span>
            </label>
          ))}
        </div>
      </div>
...
export default function Usuarios() {
  ...
  const [confirmacion, setConfirmacion] = useState({
    abierto: false, titulo: "", mensaje: "", tipo: "danger", onConfirmar: null,
  });

  const [formulario, setFormulario] = useState({
    nombre: "", email: "", contraseña: "", procesos: [],
  });
  ...
  const obtenerUsuarios = async () => {
    try {
      ...
      setUsuarios(data.data.map((u) => ({ ...u, habilitado: u.estado === "Activo" })));
  ...
  const abrirModalEditar = (usuario) => {
    ...
    setFormulario({
      ...
      contraseña: usuario.contraseña, procesos: usuario.procesos || [],
    });
    setMostrarContraseña(false);
    setModalEditarAbierto(true);
  };
  const cerrarModalEditar = () => { setModalEditarAbierto(false); setUsuarioEditando(null); };

  // ── Crear usuario ─────────────────────────────────────────────────
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (formulario.procesos.length === 0) {
      toast.error("Debe seleccionar al menos un sector para el operario."); return;
    }
    try {
      const res = await fetch(`${API_URL}/usuarios`, {
        method: "POST",
  ...
  const handleSubmitEditar = async (e) => {
    ...
    if (formulario.contraseña && formulario.contraseña.length < 5) {
      toast.error("La contraseña debe tener entre 5 y 8 caracteres."); return;
    }
    const payload = {
      nombre: formulario.nombre, email: formulario.email,
  ...
  const usuariosFiltrados = filtro === "habilitadas"
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-bold text-gray-900 text-xl sm:text-2xl">Gestión de Usuarios</h1>
          <p className="text-gray-600 mt-1 text-sm sm:text-base">Administra los operarios del sistema</p>
        </div>
        <button
          onClick={abrirModal}
      ...
      <Card>
        <CardContent className="pt-4 pb-4 sm:pt-6 sm:pb-6">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
            <span className="text-xs sm:text-sm font-medium text-gray-700">Filtrar por:</span>
            <div
        ...
        {usuarios.length === 0 ? (
          ...
          <div className="divide-y divide-gray-200">
            {usuariosFiltrados.map((usuario) => (
              <div key={usuario.id_usuario} className="p-4 sm:p-6 hover:bg-gray-50 transition-colors">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h3 className="font-bold text-gray-900 text-sm sm:text-base truncate">{usuario.nombre}</h3>
                      <div className="space-y-1 text-xs sm:text-sm mt-1">
                        <p className="text-gray-600 truncate"><span className="font-medium">Correo:</span> {usuario.email}</p>
                        <p className="text-gray-600"><span className="font-medium">Contraseña:</span> {usuario.contraseña}</p>
                        <p className="text-gray-600">
                          <span className="font-medium">Rol:</span>{" "}
                          <span className="inline-flex px-2 py-0.5 bg-purple-100 text-purple-700 text-xs rounded">Operario</span>
                        </p>
        ...
        <ModalOverlay onClose={cerrarModal}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-5 sm:p-6 border-b border-gray-200 flex-shrink-0">
              <h2 className="font-bold text-gray-900 text-lg sm:text-xl">Crear Nuevo Operario</h2>
      ...
      {modalEditarAbierto && (
        <ModalOverlay onClose={cerrarModalEditar}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-5 sm:p-6 border-b border-gray-200 flex-shrink-0">
              <h2 className="font-bold text-gray-900 text-lg sm:text-xl">Editar Operario</h2>
